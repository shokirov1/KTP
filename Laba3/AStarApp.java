 ...
 * местоположение. Пользователь может рисовать или устранять препятствия на
 * карте, а затем нажимать кнопку, чтобы вычислить путь от начала до конца с
 * использованием алгоритма поиска пути A *. Если путь найдено, отображается
 * зеленым цветом.
 **/
public class AStarApp {
    ...
    /**
     * Это двумерный массив компонентов пользовательского интерфейса, которые обеспечивают отображение и управление
     ** ячейки на карте.
     ***/
    private JMapCell[][] mapCells;

    /**
     * Этот внутренний класс обрабатывает события мыши в основной сетке ячеек карты с помощью
     * изменение ячеек на основе состояния кнопки мыши и начального редактирования, которое
     * было выполнено.
     **/
    private class MapCellHandler implements MouseListener {
        ...
        /**
         * Это значение фиксирует, делаем ли мы клетки проходимыми или непроходимыми. Который
         * это зависит от исходного состояния ячейки, в которой выполнялась операция
         * началось внутри.
         **/
        ...
        /**
         * Если была нажата мышь, это продолжит операцию модификации в
         * новая ячейка.
         **/
    ...
    /**
     * Простой вспомогательный метод для настройки пользовательского интерфейса Swing. Это вызвано из
     * поток обработчика событий Swing должен быть потокобезопасным.
     **/
     ...
     * Реализация довольно медленная; создается новый объект {@link Map2D} и
     * инициализируется из текущего состояния приложения. Тогда поисковик A * будет
     * вызывается, и если путь найден, дисплей обновляется, чтобы показать путь, который
     * был найден. (Лучшее решение могло бы использовать дизайн Контроллера представления модели
     * шаблон.)
     **/
    ...
    /**
     * Точка входа для приложения. Аргументы командной строки не распознаются в
     * этот раз.
     **/
